...
def dist(lat1, lat2, lng1, lng2):
    ...
    km_dist = (
        (lat1.sin()) * (lat2.sin())
        + (lat1.cos()) * (lat2.cos()) * ((lng2 - lng1).cos())
    ).arccos() * 6371
    return km_dist
...
def test_dist():
    df = pl.DataFrame(
        {"start_lat": [32], "end_lat": [33], "start_lng": [24], "end_lng": [38]}
    )
    actual = df.select(
        dist=dist(
            pl.col("start_lat"),
...
def split_data(df, train_cutoff_time):
    df_train, df_test = df.with_columns(
        train_test=pl.when(pl.col("start_time") <= train_cutoff_time)
        .then(pl.lit("train"))
...
def create_cells(df, lat_bins, lng_bins, use_quantiles: bool):
    ...
    df_grp_all = add_empty_cells(
        df_grp, ["start_lat_cut", "start_lng_cut", "start_time_cut"]
    )
...
def aggregate_to_cells(df):
    df_grp = df.group_by(["start_lat_cut", "start_lng_cut", "start_time_cut"]).agg(
        cell_count=pl.col("dist").len(),
...
def add_empty_cells(df_grp, index_cols):
    ...
    df_grp = df_grp.join(levels, on=index_cols, how="full", coalesce=True).with_columns(
...
def test_add_empty_cells():
    input = pl.DataFrame(
        {"a": [1, 1, 2, 2], "b": [1, 1, 1, 2], "cell_count": [1, 2, 3, 4]}
    )
    actual = add_empty_cells(input, ["a", "b"])

    expected = pl.DataFrame(
        {"a": [1, 1, 1, 2, 2], "b": [1, 1, 2, 1, 2], "cell_count": [1, 2, 0, 3, 4]}
    )
...
def add_coord(df):
    ...
    df = df.with_columns(
        start_lat_flr=pl.col("start_lat_cut")
        .cast(pl.String)
        .str.split(",")
        .list.get(0)
        .str.strip_chars_start("(")
        .cast(pl.Float64),
        start_lat_idx=pl.col("start_lat_cut").replace_strict(
            start_lat_cut_cats, list(range(len(start_lat_cut_cats)))
        ),
        start_lng_flr=df.get_column("start_lng_cut")
        .cast(pl.String)
        .str.split(",")
        .list.get(0)
        .str.strip_chars_start("(")
        .cast(pl.Float64),
        start_lng_idx=pl.col("start_lng_cut").replace_strict(
            start_lng_cut_cats, list(range(len(start_lng_cut_cats)))
        ),
    )
    return df
...
def add_time_features(df):
    df = df.with_columns(
        ...
        start_time_cut_time=pl.col("start_time_cut").dt.hour().cast(pl.Int64) * 60
        + pl.col("start_time_cut").dt.minute(),
    )
    return df
...
def test_add_time_features():
    input = pl.DataFrame(
        {
            "start_time_cut": [
                datetime.datetime(2012, 1, 3, 12, 17),
                datetime.datetime(2012, 1, 3, 14, 50),
            ]
        }
    )
    ...
    expected = pl.DataFrame(
        ...
        schema={
            "start_time_cut": pl.Datetime,
            "start_time_day": pl.Int8,
            "start_time_cut_time": pl.Int64,
        },
    )
...
def add_lag_features(df):
    df_lag = df.with_columns(
        [
            pl.col("cell_count")
            .shift(lag)
            .over(["start_lat_cut", "start_lng_cut"])
            .alias(f"cell_count_lag_{lag}")
            for lag in [1, 2, 3]
        ]
    )
    return df_lag
